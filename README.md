...
1. 启动 FH6，并保持窗口模式或无边框窗口模式，不要最小化；重点适配尺寸为 `1600×900` 及以上。
2. 打开 `FH6Gacha.exe`。
...
EasyOCR 模型保存在 EXE 同目录的 `.easyocr_models/`。价格区域已经由程序精确裁剪，因此只使用数字识别模型，不再携带冗余的全文检测模型和其他语言字符表。如果构建包没有内置模型，第一次需要价格的重复车会在安全时限内等待模型联网下载和初始化；下载失败会记录日志。按价格策略会保留车辆，“全部出售”仍会出售但该车收入无法计入统计。
...
环境：Windows 10/11、Python 3.10+、FH6 简体中文、Steam 版。以 `1600×900` 作为重点适配和验收尺寸；更小窗口不会被禁止，程序仍会按比例尝试，但不保证模板识别效果。
...
构建脚本会先核验依赖、预下载 EasyOCR 英文识别模型、运行完整测试，再生成 `dist\FH6Gacha.exe`，最后用 `--smoke-test` 启动打包产物，实际加载内置 OCR 模型并确认界面能正常初始化和退出。模型下载成功时会直接打进 EXE；构建阶段下载失败时仍可生成程序，并在首次按价格抽奖时重试下载。GitHub Actions 构建要求模型准备与打包后冒烟测试都成功后才会上传产物。原项目遗留的收款码和赞助图片不会显示、释放或打进 EXE。
